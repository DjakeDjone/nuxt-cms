...
export const useAuthHandler = () => {
  ...
  const getAllUsers = async (): Promise<BaseAuthUser[]> => {
    ...
    if (users.length === 0) {
      const initUsers =
        useRuntimeConfig().editableContent?.auth?.initUsers || [];
  ...
  const sanitizeUser = <PublicUserData, Roles extends string>(
    user: BaseAuthUser<PublicUserData, unknown, Roles>
  ...
  const createUserSession = async <PublicUserData, Roles extends string>(
    ...
    user: SanitizedUser<PublicUserData, Roles>;
    token: AuthToken;
  }> => {
    ...
    return {
      user: sanitizeUser<PublicUserData, Roles>(mutableUser),
      token: token,
    };
  };
