...
<template>
    <div ref="calendarRef" class="ui-calendar" tabindex="0" role="application"
        ...
        <div class="calendar-header">
            <div class="calendar-nav">
                <div class="calendar-controls">
                    <div class="calendar-controls-month">
                        <UiBtn @click="navigateMonth('prev')" aria-label="Previous month (Page Up)"
                        ...
                        <UiBtn @click="navigateMonth('next')" aria-label="Next month (Page Down)"
            ...
            <UiBtn :active="isToday" @click="goToToday" title="Go to today (T)" aria-label="Go to today (T)">
                Today {{ isToday ? '✓' : '' }}
            </UiBtn>
        </div>

        <!-- Keyboard shortcuts help -->
        <div class="keyboard-help" title="Keyboard shortcuts">
            <small>
            ...
            <div v-for="day in calendarDays" :key="day.date.getTime()" class="calendar-day" :class="{
                ...
                <div v-if="day.events.length > 0" class="day-events">
                    <div v-for="(event, index) in day.events.slice(0, 7)" :key="event.id" class="event-dot"
                        ...
                        :aria-label="`Event: ${event.title}`" role="button" tabindex="-1"
                        @click.stop="onEventClick(event)" @keydown.enter.stop.prevent="onEventClick(event)"
